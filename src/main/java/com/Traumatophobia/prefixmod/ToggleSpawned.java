...
public class ToggleSpawned implements IClientCommand {
	public static boolean Toggled = false;
	
	private final List aliases;

	public ToggleSpawned()
    { 
		aliases = new ArrayList();
		aliases.add("tspawned");
		aliases.add("togglespawn");
	...
	public void execute(MinecraftServer arg0, ICommandSender arg1, String[] arg2) throws CommandException {
		...
		if (Toggled == true) {
			Toggled = false;
			telluser.appendText("Spawned mob notifications enabled");
		}
		else if (Toggled == false) {
			Toggled = true;
			telluser.appendText("Spawned mob notifications disabled");
