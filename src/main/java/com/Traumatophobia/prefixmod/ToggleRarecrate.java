...
public class ToggleRarecrate implements IClientCommand {
	public static boolean Toggled = false;
	
	private final List aliases;

	public ToggleRarecrate()
    { 
		aliases = new ArrayList();
		aliases.add("trare");
		aliases.add("togglecrate");
	...
	public void execute(MinecraftServer arg0, ICommandSender arg1, String[] arg2) throws CommandException {
		...
		if (Toggled == true) {
			Toggled = false;
			telluser.appendText("Rare crate key notifications enabled");
		}
		else if (Toggled == false) {
			Toggled = true;
			telluser.appendText("Rare crate key notifications disabled");
