...
public class ToggleWelcome implements IClientCommand {
	public static boolean Toggled = false;
	
	private final List aliases;

	public ToggleWelcome()
    { 
		aliases = new ArrayList();
		aliases.add("twelcome");
		aliases.add("tw");
	...
	public void execute(MinecraftServer arg0, ICommandSender arg1, String[] arg2) throws CommandException {
		...
		if (Toggled == true) {
			Toggled = false;
			telluser.appendText("Island Welcomes enabled");
		}
		else if (Toggled == false) {
			Toggled = true;
			telluser.appendText("Island Welcomes disabled");
