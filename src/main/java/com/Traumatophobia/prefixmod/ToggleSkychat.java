...
public class ToggleSkychat implements IClientCommand {
	public static boolean Toggled = false;
	
	private final List aliases;

	public ToggleSkychat()
    { 
		aliases = new ArrayList();
		aliases.add("tsc");
		aliases.add("togglesc");
	...
	public void execute(MinecraftServer arg0, ICommandSender arg1, String[] arg2) throws CommandException {
		...
		if (Toggled == true) {
			Toggled = false;
			telluser.appendText("Skychat Hover game enabled");
		}
		else if (Toggled == false) {
			Toggled = true;
			telluser.appendText("Skychat Hover game disabled");
